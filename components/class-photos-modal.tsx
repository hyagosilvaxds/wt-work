...
export function ClassPhotosModal({ isOpen, onClose, turma, readOnly = false }: ClassPhotosModalProps) {
  ...
  const handleDownloadPhoto = (photo: ClassPhoto) => {
    ...
    link.href = `worktreinamentos.olimpustech.com/${photo.path}`
            ...
            <ScrollArea className="h-[500px]">
              {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {photos.map((photo) => {
                    console.log('📸 Path original:', photo.path, '| URL completa:', `worktreinamentos.olimpustech.com${photo.path}`, '| ID:', photo.id)
                    return (
                      <Card key={photo.id} className="overflow-hidden h-fit">
                        <div className="aspect-video relative bg-gray-100">
                          <img
                            src={`worktreinamentos.olimpustech.com/${photo.path}`}
