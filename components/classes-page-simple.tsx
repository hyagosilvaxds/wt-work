...
export default function ClassesPageSimple() {
  ...
  const loadClasses = async () => {
    ...
    try {
      ...
      setTotalClasses(response.pagination?.total || 0)
    } catch (error: any) {
          ...
          {classes.map((classItem) => (
            <Card key={classItem.id} className="hover:shadow-lg transition-shadow">
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <Badge className={getTypeColor(classItem.type)} variant="outline">
                        {classItem.type}
                      </Badge>
                    </div>
                    <p className="text-gray-600 font-medium">
                      {classItem.training.description}
                    </p>
                    {classItem.client && (
                      <p className="text-sm text-gray-500 mt-1">
                        Cliente: {classItem.client.name}
                      </p>
                    )}
                  </div>
                  <DropdownMenu>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="font-medium">
                        {classItem.students.length} aluno{classItem.students.length !== 1 ? 's' : ''}
                      </p>
                      <p className="text-sm text-gray-500">
                        {classItem.lessons.length} aula{classItem.lessons.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {calculateDuration(classItem.startDate, classItem.endDate)} dias
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Validade: {classItem.training.validityDays} dias
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {(classItem.location || classItem.recycling !== "NÃO" || classItem.observations) && (
                  <div className="mt-6 pt-6 border-t">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      {classItem.recycling !== "NÃO" && (
                        <div>
                          <span className="text-gray-500">Reciclagem:</span>
                          <p className="font-medium">{classItem.recycling}</p>
                        </div>
                      )}
                      {classItem.observations && (
                        <div>
                          <span className="text-gray-500">Observações:</span>
                          <p className="font-medium">{classItem.observations}</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="mt-6 pt-6 border-t flex gap-2 flex-wrap">
                  ...
                  <Button variant="outline" size="sm" className="gap-2">
                    <Calendar className="h-4 w-4" />
                    Aulas ({classItem.lessons.length})
                  </Button>
                  {classItem.status === "EM ABERTO" && (
                    <Button variant="outline" size="sm" className="gap-2">
                      <Edit className="h-4 w-4" />
                      Editar
                    </Button>
