...
import { updateVehicle, toggleVehicleStatus, type Vehicle, type UpdateVehicle } from "@/lib/api/superadmin"
...
export function VehicleEditModal({ vehicle, open, onOpenChange, onVehicleUpdated }: VehicleEditModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        ...
        toast({
          title: "Erro",
          description: error.response?.data?.message || "Erro ao atualizar veículo",
          variant: "destructive",
        })
      }
    } finally {
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      toast({
        title: "Sucesso",
        description: `Veículo ${vehicle.isActive ? 'desativado' : 'ativado'} com sucesso`,
      })
    } catch (error: any) {
      console.error('Erro ao alterar status do veículo:', error)
      toast({
        title: "Erro",
        description: error.response?.data?.message || "Erro ao alterar status do veículo",
        variant: "destructive",
      })
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Atualize os dados do veículo {vehicle.brand} {vehicle.model} - {vehicle.licensePlate}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-medium">Status do Veículo</h4>
                <p className="text-sm text-gray-600">
                  Veículo atualmente {vehicle.isActive ? 'ativo' : 'inativo'}
                </p>
              </div>
              <Button
                type="button"
              ...
              >
                {toggleLoading ? "Processando..." : vehicle.isActive ? "Desativar" : "Ativar"}
              </Button>
            </div>
          </div>
