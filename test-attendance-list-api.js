...
async function testPreviewWithStudents() {
  ...
  try {
    const response = await fetch(`http://localhost:4000/certificado/attendance-list/${lessonId}/preview/with-students`)
    
    if (response.ok) {
      console.log('✅ Preview com alunos disponível!')
      console.log(`📄 Content-Type: ${response.headers.get('content-type')}`)
      console.log(`🔗 URL: http://localhost:4000/certificado/attendance-list/${lessonId}/preview/with-students`)
    } else {
      const errorData = await response.json()
...
async function testPreviewEmpty() {
  ...
  try {
    const response = await fetch(`http://localhost:4000/certificado/attendance-list/${lessonId}/preview/empty-fields`)
    
    if (response.ok) {
      console.log('✅ Preview vazia disponível!')
      console.log(`📄 Content-Type: ${response.headers.get('content-type')}`)
      console.log(`🔗 URL: http://localhost:4000/certificado/attendance-list/${lessonId}/preview/empty-fields`)
    } else {
      const errorData = await response.json()
