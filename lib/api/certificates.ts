...
export const getCompletedClassesFiltered = async (filters: CompletedClassesFiltersDto = {}): Promise<CompletedClassesResponseDto> => {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://api.olimpustech.com.br'
...
export const checkClassEligibility = async (classId: string): Promise<boolean> => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://api.olimpustech.com.br'}/certificado/eligibility/${classId}`, {
      headers: {
