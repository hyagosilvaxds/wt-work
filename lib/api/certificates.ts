...
export const generateEvidenceReport = async (classId: string): Promise<void> => {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'api.olimpustech.com'
...
export const checkClassEligibility = async (classId: string): Promise<boolean> => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'api.olimpustech.com'}/certificado/eligibility/${classId}`, {
      headers: {
